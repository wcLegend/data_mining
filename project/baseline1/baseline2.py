...
fh = logging.FileHandler("./baseline2.log", mode="w")
...
def fea_importance_get(): #筛选特征值,把重要性小于一定值的特征提取出来
    a = pd.read_csv('feature_score_2.csv', header=0)
    #train.columns = ['feature', 'importance']
    a = a[a.importance<100].iloc[:,0]
    return a

fea_importance = fea_importance_get()

base_path = ''
# 加载邀请回答数据

train = pd.read_csv(base_path + 'invite_info_0926.txt', sep='\t', header=None)
train.columns = ['qid', 'uid', 'dt', 'label']
logging.info("invite %s", train.shape)
logger.info("invite %s", train.shape)

test = pd.read_csv(base_path + 'invite_info_evaluate_2_0926.txt', sep='\t', header=None)
...
# 加载问题
ques = pd.read_csv(base_path + 'question_info_0926.txt', header=None, sep='\t')
...
# 加载回答
ans = pd.read_csv(base_path + 'answer_info_0926.txt', header=None, sep='\t')
...
# train
# val
train_start = train['day'].min()  # train中邀请的最早时间
train_end = train['day'].max()  # train中邀请的最晚时间

val_start = test['day'].min()  # val中邀请的最早时间
val_end = test['day'].max()  # val中邀请的最晚时间

label_end = train_end  # train中邀请的最晚时间 #3867

answer_start = ans['a_day'].min()
answer_end = ans['a_day'].max()

train_label_feature_end = label_end - (val_end - val_start) - 1
train_label_feature_start = train_start

train_ans_feature_end = train_label_feature_end
train_ans_feature_start = answer_start

val_label_feature_end = val_start - 1  
val_label_feature_start = val_label_feature_end - train_start  

val_ans_feature_end = val_start - 1
val_ans_feature_start = val_ans_feature_end - answer_start


"""
# 时间窗口划分    目的是把train和ans数据集根据时间窗口划分训练测试集
# train
# val
train_start = 3838
train_end = 3867
...
logger.info("train_label_feature %s", train_label_feature.shape)
...
logger.info("val_label_feature %s", val_label_feature.shape)
...
logger.info("train feature start %s end %s, label start %s end %s", train_label_feature['day'].min(),
            train_label_feature['day'].max(), train_label['day'].min(), train_label['day'].max())
...
logger.info("test feature start %s end %s, label start %s end %s", val_label_feature['day'].min(),
            val_label_feature['day'].max(), test['day'].min(), test['day'].max())
...
logger.info("train ans feature %s, start %s end %s", train_ans_feature.shape, train_ans_feature['a_day'].min(),
            train_ans_feature['a_day'].max())

logging.info("val ans feature %s, start %s end %s", val_ans_feature.shape, val_ans_feature['a_day'].min(),
             val_ans_feature['a_day'].max())
logger.info("val ans feature %s, start %s end %s", val_ans_feature.shape, val_ans_feature['a_day'].min(),
            val_ans_feature['a_day'].max())
...
"""
#这里我删去是否含有图片，我认为这个并不影响
fea_cols = ['is_good', 'is_rec', 'is_dest', 'word_count',
            'reci_cheer', 'reci_uncheer', 'reci_comment', 'reci_mark', 'reci_tks',
            'reci_xxx', 'reci_no_help', 'reci_dis', 'diff_qa_days']
...
def extract_feature1(target, label_feature, ans_feature):
    # 问题特征
    t1 = label_feature.groupby('qid')['label'].agg(['mean', 'sum', 'std', 'count']).reset_index() #问题被回答的总数和问题在数据集中被提到的总数,平均数可以代表回答概率
    ...
    for col in fea_cols:
        t1 = ans_feature.groupby('uid')[col].agg(['sum', 'max', 'mean','min','std']).reset_index()
        t1.columns = ['uid', 'u_' + col + '_sum', 'u_' + col + '_max', 'u_' + col + '_mean','u_' + col + '_min','u_' + col + '_std']
        target = pd.merge(target, t1, on='uid', how='left')

        t1 = ans_feature.groupby('qid')[col].agg(['sum', 'max', 'mean','min','std']).reset_index()
        t1.columns = ['qid', 'q_' + col + '_sum', 'q_' + col + '_max', 'q_' + col + '_mean','q_' + col + '_min','q_' + col + '_std']
        target = pd.merge(target, t1, on='qid', how='left')
        logging.info("extract %s", col)
        logger.info("extract %s", col)
    return target
...
logger.info("train shape %s, test shape %s", train_label.shape, test.shape)
assert len(test) == sub_size

# 加载用户
user = pd.read_csv(base_path + 'member_info_0926.txt', header=None, sep='\t')
...
for x in unq[unq == 1].index: #删去那些只有一个值的特征
    del user[x]
...
for d in cats:
    lb = LabelEncoder()  #这个的作用就是把不连续的整数变为连续的整数
...
for feat in count_fea:
    ...
    data[col_name] = data[feat].map(data[feat].value_counts().astype(int)) #将总的数值数作为该特征的值
    data.loc[data[col_name] < 2, feat] = -1#将仅有一次记录的属性值设置为-1？
    data[feat] += 1 
    data[col_name] = data[feat].map(data[feat].value_counts().astype(int))
    data[col_name] = (data[col_name] - data[col_name].min()) / (data[col_name].max() - data[col_name].min())
    #无量纲化处理，可以当做是一种标准化的处理
...
logger.info("feature size %s", len(feature_cols))
...
#删除不重要特征
"""
for fea in fea_importance.values:
    print(fea)
    del X_train[fea]
    del X_val[fea]
    feature_cols.remove(fea)
    pass
...
a = sum(y_train==1)
b = sum(y_train==0)
print('a',a,'  b',b,'rate_new====: ',b/a)
#'''


model_lgb = LGBMClassifier(n_estimators=2000,n_jobs=-1, objective='binary', seed=1000, silent=True)
#model_lgb = LGBMClassifier(n_estimators=2000,max_bin=100,num_leaves=20, objective='binary', seed=500, silent=True,learning_rate =0.1,n_jobs=-1)
...
print('result===============')
sub.to_csv('result_base2_last_v2.txt', index=None, header=None, sep='\t') #2.3 调整时间窗口  2.4  调整学习率  2.5 特征筛选350


print('lgbm的特征重要性')
#print('Feature importances:', list(model_lgb.feature_importances_))

df['importance']=list(model_lgb.feature_importances_)
df = df.sort_values(by='importance',ascending=False)
df.to_csv("feature_score_last_v2.csv",index=None,encoding='gbk')
