...
# base_path = '../data'
#base_path = '../../share/liuyuchao/EFdata/data/'
base_path = ''
# 加载邀请回答数据
...
clf = xgb.train(params, trn_data, 1000, watchlist, verbose_eval=200, early_stopping_rounds=100)

##预测部分
sub['label'] = clf.predict(xgb.DMatrix(test[feature_cols]), ntree_limit=clf.best_ntree_limit)[:, 1]


print('result===============')
sub.to_csv('xgboost_import.txt', index=None, header=None, sep='\t')
...
fold_importance_df.to_csv("feature_score_20180405.csv",index=None,encoding='gbk')
