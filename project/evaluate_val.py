...
if __name__ == "__main__":
	...
	for epoch in range(num_iters):
		...
		for i in range(0, fold_size, batch_size):
			...
			print(len(data.val_x))
			val_inseq = data.val_x[i : i + batch_size]
			...
			print(pred)		
			print(np.array(pred.detach())[:,1])
			data.test['label'] = np.array(pred.detach())[:,1]
		
		
		break
	data.test.to_csv('result.txt', index=None, header=None, sep='\t')
