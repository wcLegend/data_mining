...
import sys
import torch
from data_preprocess_test import Data_Preprocess
...
if __name__ == "__main__":
	...
	for epoch in range(num_iters):
		...
		for i in range(0, fold_size, batch_size):
			...
			print('result=====',ans)			
			res = ans.view(batch_size) - train_out
			res = batch_size-sum(abs(res.cpu().numpy())) 
			print('正确的个数=====',res)


			loss= mlp_criterion(pred, train_out)
			loss_all += loss
			print('i: ',i,' ==loss: ',loss)
		...
		torch.save(wEncoder.state_dict(), 'model/wencoder_epoch_{}.pt'.format(epoch))
		torch.save(swEncoder.state_dict(), 'model/swencoder_epoch_{}.pt'.format(epoch))
		torch.save(mlp_a.state_dict(), 'model/mlp_epoch_{}.pt'.format(epoch))
