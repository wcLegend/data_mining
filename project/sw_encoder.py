...
class swEncoder(nn.Module):
    """docstring for Encoder"""
    def __init__( self, hidden_size,sword2vec,sword_size, num_layers=1, batch_size=1):
        super(swEncoder, self).__init__()
        ...
        self.input_size = sword_size
        self.embedding  = nn.Embedding(self.input_size, sword2vec.size()[1])
        self.embedding.from_pretrained(sword2vec)
    ...
    def forward(self, input):
        ...
        print('sw_encoder embedding input ',input.size())
        embedded = self.embedding(input) # L, B, V
        batch_size = input.size()[0]
        #packed = torch.nn.utils.rnn.pack_padded_sequence(embedded, input_lengths)
        hidden = self.init_hidden(batch_size)
        print('sw_encoder gru hidden',hidden.size())
