...
class Data_Preprocess(object):
    ...
    def get_use_dic(self):
        ...
        #user_info = pd.read_csv('member_info_0926_s.txt', header=None, sep='\t')
    ...
    def load_train(self):
        #invite = pd.read_csv('invite_info_0926_50.txt', header=None, sep='\t')
        ...
        invite = pd.read_csv('invite_info_evaluate_1_0926.txt', header=None, sep='\t')
        invite.columns = ['问题id', '用户id', '邀请创建时间']
        self.test = invite.copy()
        for row in invite.values:
            self.val_x.append(row[0:2])
            pass
            #break
        pass
        #print(self.train_x)
